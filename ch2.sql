...
SELECT s.year, CONCAT(UPPER(s.name), '!!!') as names_cap 
FROM sets s
...
--  Select the name and year column. Construct an additional column in your SELECT clause using the MySQL MAKEDATE() function. This column's value should be a date corresponding to January 1st of the year. 
